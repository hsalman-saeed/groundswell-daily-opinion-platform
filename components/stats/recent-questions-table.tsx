...
export function RecentQuestionsTable({ questions = [] }: { questions?: any[] }) {
  const [open, setOpen] = useState<number | null>(null)

  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      {/* Header */}
      <div className="hidden grid-cols-[4.5rem_6.5rem_minmax(0,1fr)_4rem_5.5rem_5rem] gap-3 border-b border-border bg-surface px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground md:grid">
        <span>Date</span>
        <span>Topic</span>
        <span>Question</span>
            ...
            return (
              <div key={i}>
                <button
                  ...
                  className="grid w-full grid-cols-[1fr_auto] items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-surface md:grid-cols-[4.5rem_6.5rem_minmax(0,1fr)_4rem_5.5rem_5rem]"
                >
                ...
                {isOpen && q.breakdown && q.breakdown.length > 0 && (
                  <div className="gs-fade-in border-t border-border bg-surface px-4 py-4">
                    ...
                    <div className="flex flex-col gap-2">
                      {q.breakdown.map((b: any) => {
                        const err = parseFloat(Math.abs(b.actual - b.predicted).toFixed(1))
                        return (
                          <div
                            key={b.label}
